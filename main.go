...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		//cli.StringFlag{Name: "single,s", Value: "", Usage: "tvdatatools --single XXX.csv"},
	}
	...
	app.Action = func(c *cli.Context) {
		...
		if len(infile) != 0 {
			...
			fmt.Println(<-OTTVodData(infile))
		} else {
			fmt.Println("Input file is not exist!")
			fmt.Println("Usage: tvdatatools --in XXX.csv")
		}
...
//分析和处理OTT 点播数据
//输入:infile 点播数据文件名
func OTTVodData(infile string) <-chan string {
	c := make(chan string)
	go func() {
		//获取文件名前缀
